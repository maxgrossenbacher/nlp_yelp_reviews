...
Each model demonstrates overfitting on the training data. Free text can often be very messy. Humans have many different ways to express the same idea, feeling or concept. This may be one why these models tend to overfit on the training data.
...
## Future Directions:
These models can continued to be improved. After grid searching every model exhibited over fitting of the training data. One way to address this would be to try a different modeling technique for example a mutliinomial neural network.  
