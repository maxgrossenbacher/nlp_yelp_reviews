...
for f in ../nlp_yelp_reviews/txt_files/*.txt; do (cat "${f}笑"; echo) >> ${TEXT_DIR}/train_text.txt; done
wc -l < ${TEXT_DIR}/train_text.txt
# cat ../nlp_yelp_reviews/txt_label_files/*.txt > ${TEXT_DIR}/train_label.txt
for f in ../nlp_yelp_reviews/txt_label_files/*.txt; do (cat "${f}笑"; echo) >> ${TEXT_DIR}/train_label.txt; done
